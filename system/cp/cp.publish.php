...
class Publish {
    ...
    function new_entry_form($which = 'new', $submission_error = '', $entry_id='', $hidden = array())
    {
		...
		if ($PREFS->ini('publish_page_title_focus') !== 'n')
		{
...
EOT;
		...
		if ($show_forum_cluster == 'y' AND $PREFS->ini('forum_is_installed') == "y")
		{
			...
			if ($which == 'new' OR $entry_id == '')
			{
				...
				$fquery = $DB->query("SELECT forum_id, forum_name FROM exp_forums WHERE forum_is_cat = 'n' ORDER BY forum_order asc");
				...
				else
				{
					...
					$field_js = ($show_button_cluster == 'y') ? "onclick='setFieldName(this.name)'" : '';
                    ...
                    {                           
                        $r .= $DSP->input_select_option($forum['forum_id'], $forum['forum_name'], (($forum['forum_id'] == $IN->GBL('forum_id')) ? 1 : ''));
                    }
    ...
    function submit_new_entry($cp_call = TRUE)
    {
        ...
        if ( ! $title = strip_tags(trim($IN->GBL('title', 'POST'))))
        {
			...
			if (! isset($orig_author_id) OR $author_id != $orig_author_id)
			{
				...
				{				
					...
					$ss = "SELECT exp_members.member_id
						   ...
						   WHERE exp_members.in_authorlist = 'y' 
						   AND exp_member_groups.site_id = '".$DB->escape_str($PREFS->ini('site_id'))."'";
			...
			is_numeric($IN->GBL('pages_template_id', 'POST')))
		{
			...
			$static_pages['uris'][$entry_id]      = '/'.trim(preg_replace("#[^a-zA-Z0-9_\-/\.]+$#i", '', str_replace($PREFS->ini('site_url'), '', $IN->GBL('pages_uri'))), '/').'/';
			$static_pages['templates'][$entry_id] = preg_replace("#[^0-9]+$#i", '', $IN->GBL('pages_template_id', 'POST'));
			
			if ($static_pages['uris'][$entry_id] == '//')
			{
				$static_pages['uris'][$entry_id] = '/';
			}
        ...
        {
			$loc = ($return_url == '') ? $FNS->fetch_site_index() : $FNS->create_url($return_url, 1, 0);
        }
...
EOT;
		...
		if ($query->num_rows > 0)
		{
			foreach($query->result as $row)
			{
				...
				if ($IN->GBL('type') == 'add')
				{
					...
					foreach($ids as $id)
					{
						$DB->query($DB->insert_string('exp_category_posts', array('entry_id' => $id, 'cat_id' => $row['cat_id'])));
					}
				}
			}
		}
    ...
    function view_entry()
    {
        global $DSP, $LANG, $FNS, $DB, $IN, $REGX, $SESS, $EXT, $LOC;
