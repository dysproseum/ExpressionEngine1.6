...
<?php
    ...
    {  
		...
		if (in_array($_POST['template_type'], array('css', 'js', 'rss', 'static', 'weboage', 'xml')))
		{
...
<?php
        ...
        else
        {
			...
			foreach ($query->result as $row)
			{
				...
				$hidden = (substr($row['template_name'], 0, 1) == '.') ? '_hidden' : '';
    ...
    function create_new_template()
    {
        ...
        {
        	$_POST['library'] = $FNS->filename_security($_POST['library']);
