...
class Utilities {
	...
	{
        global $DSP, $IN, $PREFS, $LANG, $DB, $FNS;
        ...
        if (count($extension_files) > 0)
        {
            ...
            foreach ($extension_files as $extension_name)
            {
				...
				if ($PREFS->ini('allow_extensions') == 'y' && isset($extensions_installed[$extension_name]))
				{
					...
					if ($version > $extensions_installed[$extension_name] && method_exists($OBJ, 'update_extension') === TRUE)
        			{
        				$update = $OBJ->update_extension($extensions_installed[$extension_name]);
    	    		}
