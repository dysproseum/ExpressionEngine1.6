...
class Display {
    ...
    */
	...
	function right_crumb($title, $url = '', $extra = '', $pop = FALSE)
	{
		...
		if ($url != '')
		{
			$this->rcrumb = $this->anchor($url, '<div class="crumblinksR" id="rcrumb" '.$js.'>'.$title.'</div>');
		}
		else
		{
			$this->rcrumb = $this->anchor('javascript:nullo();', '<div class="crumblinksR" id="rcrumb" '.$js.'>'.$title.'</div>');
		}
	}
	/* END */
    ...
    function copyright()
    {
        ...
        "<div class='copyright'>". $logo.$this->nl(2).$this->br().$this->nl().
         $this->anchor($FNS->fetch_site_index().$qm.'URL=http://expressionengine.com/', APP_NAME.$core." ".APP_VER)." - &#169; ".$LANG->line('copyright')." 2003 - 2007 - EllisLab, Inc.".BR.NL.
         $buyit.
...
//-->
	...
	function html_attribute_prep($label, $quotes = ENT_QUOTES)
	{
		...
		return htmlspecialchars($label, $quotes, $PREFS->ini('charset'));
	}
	/* END */
