...
class Member {
	...
	function _prep_request()
	{
		...
		elseif (preg_match("#\/#", $this->request))
		{			
	...
	function member_delete()
	{
		...
		if ($SESS->userdata['mbr_delete_notify_emails'] != '')
		{
			...
			if (preg_match('/'.$SESS->userdata('email').'/i', $notify_address))
			{
	...
	function custom_profile_data()
	{
        ...
        foreach ($query->result as $row)
        {
            ...
            foreach ($TMPL->var_single as $key => $val)
            {
				...
				if ($key == 'url')
				{
					if (substr($default_fields['url'], 0, 4) != "http" AND ! preg_match('://', $default_fields['url'])) 
