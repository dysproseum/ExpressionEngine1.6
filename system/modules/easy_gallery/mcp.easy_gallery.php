...
/*
=====================================================
 ExpressionEngine - by Laxa Inc.
-----------------------------------------------------
 http://www.laxa.ca/
-----------------------------------------------------
 Copyright (c) 2007 Laxa, Inc.
=====================================================
 THIS IS COPYRIGHTED SOFTWARE
 PLEASE READ THE LICENSE AGREEMENT
 http://laxa.ca/license.html
=====================================================
 File: mcp.easy_gallery.php
-----------------------------------------------------
 Purpose: Easy Gallery Module - CP
=====================================================
*/
...
class Easy_gallery_CP {
    ...
    function Easy_gallery_CP()
    {
        ...
        $query = $DB->query("SELECT module_version FROM exp_modules WHERE module_name = 'Easy_gallery'");
        if ($query->num_rows == 0)
        {
        	return;
        }
		...
		$this->settings['theme_path']			= PATH_THEMES.'easy_gallery_themes/';
		$this->settings['theme_url'] 			= $PREFS->ini('theme_folder_url', 1).'easy_gallery_themes/';

		# Set the module base path for convenience
        $this->base_url = BASE.'&C=modules&M=easy_gallery';
		$this->base_path = PATH.'modules/easy_gallery/';
...
*/
		...
		if (!empty($entry_id))
		{
			$sql_query = 'SELECT * FROM exp_easy_gallery WHERE entry_id="'.$IN->GBL('entry_id').'"';
	...
	function ext_gallery_images_action() 
	{
		...
		switch($IN->GBL('A')) {
			case 'image_cover': 
				...
				$sql_query = 'UPDATE exp_easy_gallery_images SET cover="No" WHERE entry_id="'.$IN->GBL('entry_id').'"';
				$DB->query($sql_query);

				# Set selected Image to be the cover
				$sql_query = 'UPDATE exp_easy_gallery_images SET cover="Yes" WHERE image_id="'.$IN->GBL('image_id').'"';
				$DB->query($sql_query);
			...
			case 'image_caption': 
				# Set selected Image to be the cover
				$sql_query = 'UPDATE exp_easy_gallery_images SET description="'.$IN->GBL('caption').'" WHERE image_id="'.$IN->GBL('image_id').'"';
				$DB->query($sql_query);

				echo 'alert("Successfully Set Caption");';
				exit;
				break;
			case 'image_order': 
				# Set selected Image to be the cover
				$sql_query = 'UPDATE exp_easy_gallery_images SET order_id="'.$IN->GBL('order_id').'" WHERE image_id="'.$IN->GBL('image_id').'"';
				$DB->query($sql_query);
				exit;
				break;
			case 'image_delete': 
				...
				# Set selected Image to be the cover
				$sql_query = 'SELECT filename FROM exp_easy_gallery_images WHERE image_id="'.$IN->GBL('image_id').'"';
...
#						echo 'alert("Image '.$files_path.$file.' does not exist.");';
				...
				$sql_query = 'DELETE FROM exp_easy_gallery_images WHERE image_id="'.$IN->GBL('image_id').'"';
				$DB->query($sql_query);
		...
		$sql_query = 'SELECT filename FROM exp_easy_gallery_images WHERE entry_id = "'.$IN->GBL('entry_id').'"';
		$results = $DB->query($sql_query);
...
//				echo 'alert("data: '.addslashes($data).'");';
	...
	function ext_gallery_images() 
	{
		...
		$sql_query = 'SELECT * FROM exp_easy_gallery';
		$results = $DB->query($sql_query);
		foreach($results->result as $fields) {
		...
		foreach($results->result as $fields) {
			...
			'<img src="'.$files_url.$filename.'_thumb.'.$file_extension.'" border="0" width="100px" height="100px" alt="'.addslashes($fields['description']).'"><br /> '.NL.
...
#			echo 'what: '.$files_path.$filename_small.BR;
			...
			$r .= 'Caption: <input type="text" name="order_id" value="'.$fields['description'].'" onblur="mod_gallery_image_caption(\''.$fields['entry_id'].'\',\''.$fields['image_id'].'\',this.value)" style="width:90px; height:12px; font-family:Arial, Helvetica, sans-serif; font-size:10px;"> <br /> '.NL;
...
//			$r .= '<img src="'.$files_url.$fields['filename'].'" border="0" class="sortable_item" id="image_'.$fields['image_id'].'" onclick="gallery_action(\'alert\',\''.$fields['image_id'].'\')" >';
//			onclick="gallery_action(\'alert\',\''.$fields['image_id'].'\')"
...
EOT;
	...
	function ext_gallery_settings_action() 
	{
		...
		{
			$sql_query = $DB->update_string('exp_easy_gallery', $field_values, 'entry_id = "'.$IN->GBL('entry_id').'"');
#			echo 'update: '.$sql_query.'<BR>';;
			$DB->query($sql_query);
		}
		else
		{
			$sql_query = $DB->insert_string('exp_easy_gallery', $field_values);
#			echo 'insert: '.$sql_query.'<BR>';;
	...
	function dashboard()
	{
        ...
        $DSP->crumb .= $DSP->anchor(BASE.AMP.'C=modules'.AMP.'M=easy_gallery', $LANG->line('easy_gallery_module_name'));
		...
		$DSP->body .= '<h5>Easy Gallery Information</h5>'.BR.NL;
...
//        									'directory'	=> array('r', array('entry_id' => 'entry_id', 'site_id' => 'site_id', 'weblog' => 'weblog'))
	...
	function settings_update()
	{
		...
		if (count($error) > 0)
		{
			$msg = $DSP->qdiv('itemWrapper', $LANG->line('easy_gallery_empty_fields'));
		...
		foreach($_POST as $key => $val) {
			$results = $DB->query('SELECT * FROM exp_easy_gallery_settings WHERE name = "'.$key.'"');
			$field_value = array('name' => $key, 'value' => $val);

			if ($results->num_rows > 0)
			{
				$sql_query = $DB->update_string('exp_easy_gallery_settings', $field_value, 'name = "'.$key.'"');
#				echo 'update: '.$sql_query.'<BR>';;
				$DB->query($sql_query);
			}
			else
			{
				$sql_query = $DB->insert_string('exp_easy_gallery_settings', $field_value);
#				echo 'insert: '.$sql_query.'<BR>';;
				$DB->query($sql_query);
			}
		}
...
#			echo 'insert: '.$sql_query.'<BR>';;
		...
		# Send message header and message if any      
		$FNS->redirect(BASE.AMP.'C=modules'.AMP.'M=easy_gallery'.AMP.'P=settings'.AMP.'msg=settings_general_updated');
		exit;        
	}
	/* END */
...
EOT;
	...
	function _generate_form($form,$form_notes) {
		...
		$output = $DSP->form_open(array('action' => 'C=modules'.AMP.'M=easy_gallery'.AMP.'P=settings_update','style' => 'margin-top:10px;'));
	...
	#########################
    # Easy_gallery Header and Message Handler
	#########################
	...
	#########################
/*
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'submit_post')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'delete_post')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'change_status')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'move_topic')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'delete_subscription')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'display_attachment')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'do_merge')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'do_split')";
        $sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'set_theme')";
		$sql[] = "INSERT INTO exp_actions (action_id, class, method) VALUES ('', 'Easy_gallery', 'do_report')";
*/


    function easy_gallery_module_install()
    {
        global $DB, $OUT, $LANG, $FNS;
          
        if ( ! is_writable(CONFIG_FILE))
        {
			$LANG->fetch_language_file('easy_gallery_cp');
        
        	return $OUT->fatal_error($LANG->line('config_not_writable'));
        }
                
        $sql[] = "INSERT INTO exp_modules (module_id, module_name, module_version, has_cp_backend) VALUES ('', 'Easy_gallery', '$this->version', 'y')";

		$sql[] = "CREATE TABLE exp_easy_gallery_settings (
		...
		$sql[] = "CREATE TABLE exp_easy_gallery ( 
		...
		$sql[] = "CREATE TABLE exp_easy_gallery_images ( 
    ...
    function easy_gallery_module_deinstall()
    {
        global $DB;    

        $query = $DB->query("SELECT module_id FROM exp_modules WHERE module_name = 'Easy_gallery'"); 
                
        $sql[] = "DELETE FROM exp_module_member_groups WHERE module_id = '".$query->row['module_id']."'";        
        $sql[] = "DELETE FROM exp_modules WHERE module_name = 'Easy_gallery'";
        $sql[] = "DELETE FROM exp_actions WHERE class = 'Easy_gallery'";
        $sql[] = "DELETE FROM exp_actions WHERE class = 'Easy_gallery_CP'";
        $sql[] = "DROP TABLE IF EXISTS exp_easy_gallery";
        $sql[] = "DROP TABLE IF EXISTS exp_easy_gallery_settings";
        $sql[] = "DROP TABLE IF EXISTS exp_easy_gallery_images";
		...
		Admin::append_config_file('', array('easy_gallery_is_installed', 'easy_gallery_trigger'));
