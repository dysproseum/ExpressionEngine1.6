...
class Template {
    ...
    function Template()
    {
    	...
    	{
    		$this->sites[$PREFS->ini('site_id')] = $PREFS->ini('site_label');
    	}
    ...
    function process_sub_templates($template)
    {
		...
		{ 
			...
			if (in_array($site_id.':'.$ex['0'].'/'.$ex['1'], $this->templates_sofar))
			{
				$this->final_template = ($PREFS->ini('debug') >= 1) ? $LANG->line('template_loop') : "";
				return;
			}
			...
			if (stristr($this->template, LD.'embed'))
			{
    ...
    function parse_template_uri()
    {
		...
		// Is the first segment the name of a tempmlate group?
		...
		if ($query->num_rows == 0)
		{
			...
			if ($PREFS->ini('site_404') == '')
			{
				...
				$tempmlate_group	= $query->row['group_name'];
				
				// Is the first segment the name of a tempmlate?			
    ...
    {    
        ...
        $charset 	= '';
        $lang		= '';
		$user_vars	= array('member_id', 'group_id', 'group_description', 'member_group', 'username', 'screen_name', 'email', 'ip_address', 'location', 'total_entries', 'total_comments', 'private_messages', 'total_forum_posts', 'total_forum_topics', 'total_forum_replies');            
        ...
        $str = preg_replace("/".LD."\s*stylesheet=[\042\047]?(.*?)[\042\047]?".RD."/", $FNS->fetch_site_index().$qs."css=\\1", $str);
	...
	function advanced_conditionals($str)
	{
		...
		$user_vars	= array('member_id', 'group_id', 'group_description', 'username', 'screen_name', 
