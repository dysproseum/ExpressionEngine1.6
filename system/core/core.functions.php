...
class Functions {  
    ...
    function fetch_assigned_weblogs($all_sites = FALSE)
    {
        ...
        else
        {
			...
			{
				$allowed_blogs = array_flip($SESS->userdata['assigned_weblogs']);
			}
    ...
    {  
    	...
    	if (sizeof($this->action_ids) == 0) return $str;
	...
	{        
		...
		for ($i = 0; $i < $total_conditionals; $i++)
		{
			...
			if (preg_match_all("/".$LD."if(\d.*?)".$RD."/", $text_range, $depth_check))
			{
				...
				$conds[] = array($LD.'if'.$eek['1'][$i].$RD, sizeof($depth_check['0']));	
			}
		}
